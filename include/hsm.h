...
#define HSM_STD_VECTOR bn::vector
...
// because GetTypeName<T> may return two different strings for the same T in different translation units.
    struct StateTypeId {
        explicit StateTypeId(const hsm_char *aStateName = nullptr) : mStateName(aStateName) {}

        hsm_bool operator==(const StateTypeId &rhs) const {
            HSM_ASSERT_MSG(mStateName != nullptr, "StateTypeId was not properly initialized");
...
// State creation interface
    struct StateFactory {
        [[nodiscard]] virtual StateTypeId GetStateType() const = 0;

        [[nodiscard]] virtual const char *GetStateName() const = 0;

        [[nodiscard]] virtual State *AllocateState() const = 0;
    };
...
// functions (below) and stored within Transition instances.
    ...
    struct ConcreteStateFactory : StateFactory {
        [[nodiscard]] StateTypeId GetStateType() const override {
        ...
        [[nodiscard]] State *AllocateState() const override {
...
// This allows us to provide an overload of transition functions that accept a state override with args.
    ...
    struct StateOverride {
        ...
        explicit operator const StateFactory &() const { return mStateFactory; }
...
// to be copyable and lightweight.
    struct Transition {
        ...
        Transition()
                : mTransitionType(Transition::No), mStateFactory(nullptr) {
        }
        ...
        [[nodiscard]] Transition::Type GetTransitionType() const { return mTransitionType; }

        [[nodiscard]] StateTypeId GetTargetStateType() const {
                    HSM_ASSERT(mStateFactory != nullptr);
            return mStateFactory->GetStateType();
        }

        [[nodiscard]] const StateFactory &GetStateFactory() const {
                    HSM_ASSERT(mStateFactory != nullptr);
            return *mStateFactory;
        }

        [[nodiscard]] const OnEnterArgsFunc &GetOnEnterArgsFunc() const { return mOnEnterArgsFunc; }

        [[nodiscard]] hsm_bool IsSibling() const { return mTransitionType == Sibling; }

        [[nodiscard]] hsm_bool IsInner() const { return mTransitionType == Inner; }

        [[nodiscard]] hsm_bool IsInnerEntry() const { return mTransitionType == InnerEntry; }

        [[nodiscard]] hsm_bool IsNo() const { return mTransitionType == No; }
...
// StateValue
    ...
    struct StateValue {
        ...
        explicit operator const T &() const { return mValue; }
    ...
    private:
        // Disable copy
        StateValue(const StateValue &rhs) = delete;

        StateValue &operator=(StateValue &rhs) = delete;
    ...
    struct StateValueResetter {
        virtual ~StateValueResetter() = default;
    };

    template<typename T>
    struct ConcreteStateValueResetter : StateValueResetter {
        explicit ConcreteStateValueResetter(StateValue<T> &stateValue) {
            mStateValue = &stateValue;
            mOrigValue = stateValue.mValue;
        }

        ~ConcreteStateValueResetter() override {
...
// State
    ...
    struct State {
        State()
                : mOwnerStateMachine(nullptr), mStackDepth(0), mStateValueResetters(0), mStateDebugName(nullptr) {
        }
        ...
        // RTTI interface
        [[nodiscard]] StateTypeId GetStateType() const { return mStateTypeId; }

        [[nodiscard]] const hsm_char *GetStateDebugName() const { return mStateDebugName; }

        // Accessors
        StateMachine &GetStateMachine() {
                    HSM_ASSERT(mOwnerStateMachine != nullptr);
        ...
        [[nodiscard]] Owner *const &GetOwner() const { return mOwner; }
        ...
        [[nodiscard]] const State *GetImmediateInnerState() const;
        ...
        template<typename StateType>
        [[nodiscard]] hsm_bool IsInState() const;

        template<typename StateType>
        [[nodiscard]] hsm_bool IsInOuterState() const { return GetOuterState<StateType>() != 0; }

        template<typename StateType>
        [[nodiscard]] hsm_bool IsInInnerState() const { return GetInnerState<StateType>() != 0; }

        template<typename StateType>
        [[nodiscard]] hsm_bool IsInImmediateInnerState() const { return GetImmediateInnerState<StateType>() != 0; }
    ...
    private:
        ...
        typedef HSM_STD_VECTOR<StateValueResetter *, 16> StateValueResetterList;
...
#define DECAY_IMMEDIATE_STRINGS_WHEN_FORWARDING
    ...
    namespace detail {
        ...
        template<size_t Nx>
        const char *DecayIfImmediateString(const char(&arr)[Nx]) {
...
// State stack types
    typedef HSM_STD_VECTOR<State *, 16> StackType;
...
// manages a stack of states.
    ...
    public:
        ...
        void Initialize(Owner *owner = nullptr) {
        ...
        [[nodiscard]] hsm_bool IsInitialized() const { return !mInitialTransition.IsNo(); }
        ...
        [[nodiscard]] const hsm_char *GetDebugName() const { return mDebugName; }

        void SetDebugTraceLevel(TraceLevel::Type trace) { mDebugTraceLevel = trace; }

        [[nodiscard]] TraceLevel::Type GetDebugTraceLevel() const { return mDebugTraceLevel; }
        ...
        [[nodiscard]] const Owner *GetOwner() const { return mOwner; }
        ...
        [[nodiscard]] const State *GetState(StateTypeId stateType) const {
            return const_cast<const State *>( const_cast<StateMachine *>(this)->GetState(stateType));
        }

        [[nodiscard]] hsm_bool IsInState(StateTypeId stateType) const { return GetState(stateType) != nullptr; }

        template<typename StateType>
        StateType *GetState() { return static_cast<StateType *>(GetState(hsm::GetStateType<StateType>())); }

        template<typename StateType>
        [[nodiscard]] hsm_bool IsInState() const { return IsInState(hsm::GetStateType<StateType>()); }
        ...
        size_t GetDebugLevel() const { return static_cast<size_t>(GetDebugTraceLevel()); }
    ...
    private:
        ...
        [[nodiscard]] const State *GetOuterState(StateTypeId stateType, size_t startDepth) const;

        State *GetInnerState(StateTypeId stateType, size_t startDepth);

        [[nodiscard]] const State *GetInnerState(StateTypeId stateType, size_t startDepth) const;
        ...
        void Log(size_t minLevel, char* type, char* message);
...
// Inline StateMachine function implementations
    ...
    inline const StateFactory &StateMachine::GetStateOverride() {
        ...
        auto iter = mStateOverrides.find(&sourceStateFactory);
...
#define HSM_LOG_TRANSITION LogTransition
    ...
    namespace detail {
        inline void
        InitState(State *state, StateMachine *ownerStateMachine, size_t stackDepth, const StateFactory &stateFactory) {
                    HSM_ASSERT(ownerStateMachine != nullptr);
    ...
    inline StateMachine::StateMachine()
            : mOwner(nullptr), mDebugTraceLevel(TraceLevel::None) {
        mDebugName[0] = '\0';
    }
    ...
    inline State *StateMachine::GetStateAtDepth(size_t depth, StateTypeId stateType) {
        ...
        return (state && state->GetStateType() == stateType) ? state : nullptr;
    }
    ...
    inline void StateMachine::Log(size_t minLevel, char* type, char* message) {
        if (static_cast<size_t>(mDebugTraceLevel) >= minLevel) {
            bn::string<4096> buffer;
            buffer = "[";
            buffer += bn::to_string<2048>(type);
            buffer += "]: ";
            buffer += bn::to_string<2048>(message);
            // Print to emulator
            bn::log(buffer);
        }
    }
    ...
    inline void StateMachine::LogTransition(size_t minLevel, size_t depth, const hsm_char *transType, State *state) {
        if (static_cast<size_t>(mDebugTraceLevel) >= minLevel) {
            bn::string<4096> buffer;
            buffer = "{";
            buffer += bn::to_string<4>(depth);
            buffer += "}: ";
            buffer += transType;
            buffer += " ";
            buffer += state->GetStateDebugName();
            bn::log(buffer);
        }
    }
